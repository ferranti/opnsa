...
<div class="c3"><a href="http://www.twitter.com/AnonymousOpNSA"><img src="img/icon-twitter.jpg" alt="TwitterLogo" /></a> <a href="http://www.youtube.com/user/AnonymousOpNSA"><img src="img/icon-youtube.jpg" alt="** PLEASE DESCRIBE THIS IMAGE **" /></a></div>
...
<tr>
<td valign="top">
<p class="c1"><span class="h3">113th US Congress</span></p>
</td>
</tr>
</table>

<!--<table class="c1" width="940" border="1" cellpadding="5">-->
<table class="c1" width="100%" border="1" cellpadding="5">
<tr>
<td width="30%">
<p class="c1"><span class="h44">Name</span></p>
</td>
<td width="10%">
<p class="c1"><span class="h44">BioGuide</span></p>
</td>
<td width="20%">
<p class="c1"><span class="h44">Phone</span></p>
</td>
<td width="5%">
<p class="c1"><span class="h44">Youtube</span></p>
</td>
<td width="5%">
<p class="c1"><span class="h44">Twitter</span></p>
</td>
<td width="10%">
<p class="c1"><span class="h44">BirthDay</span></p>
</td>
</tr>
    <xsl:for-each select="response/objects/item">
      <xsl:sort select="person/lastname"/>
       <tr>

        <td width="30%"><span class="h44">
          <xsl:element name="a">
        ...
        <td width="10%"><span class="h44">
        <xsl:element name="a">
          <xsl:attribute name="href">
            <xsl:value-of select="concat('http://bioguide.congress.gov/scripts/biodisplay.pl?index=', person/bioguideid)"/>
            </xsl:attribute>
            <xsl:attribute name="target">
             <xsl:text>_blank</xsl:text>
            </xsl:attribute>
           <xsl:element name="img">
             <xsl:attribute name="src">
                <xsl:text>GlassRockDb5.gif</xsl:text>
              </xsl:attribute>
           </xsl:element>
         </xsl:element>
        </span></td>

        <td width="20%"><span class="h44">
        <xsl:choose>
        <xsl:when test="phone !='null'">
        <xsl:element name="a">
          <xsl:attribute name="href">
           <xsl:value-of select="concat('tel:+1-', phone)"/>
          </xsl:attribute>
        ...
        <td width="5%"><span class="h44">
        <xsl:choose>
        <xsl:when test="person/youtubeid != 'null'">
          <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:value-of select="concat('https://www.youtube.com/user/', person/youtubeid)"/>
            </xsl:attribute>
          <xsl:attribute name="target">
            <xsl:text>_blank</xsl:text>
          </xsl:attribute>
          <xsl:element name="img">
            <xsl:attribute name="src">
              <xsl:text>img/icon-youtube.jpg</xsl:text>
            </xsl:attribute>
              <xsl:attribute name='alt'>
               <xsl:text>Twitter-icon</xsl:text>
              </xsl:attribute>
            </xsl:element>
          </xsl:element>
        </xsl:when>
        ...
        <td width="5%"><span class="h44">
        <xsl:choose>
        <xsl:when test="person/twitterid != 'null'">
          <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:value-of select="concat('https://www.twitter.com/', person/twitterid)"/>
            </xsl:attribute>
            <xsl:attribute name="target">
             <xsl:text>_blank</xsl:text>
            </xsl:attribute>
          <xsl:element name="img">
            <xsl:attribute name="src">
              <xsl:text>img/icon-twitter.jpg</xsl:text>
            </xsl:attribute>
              <xsl:attribute name='alt'>
               <xsl:text>Twitter-icon</xsl:text>
              </xsl:attribute>
        ...
        <td width="10%"><span class="h44">
           <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:value-of select="concat('votes_by_id/', person/id, '.xml')"/>
            </xsl:attribute>
           <xsl:element name="img">
             <xsl:attribute name="src">
                <xsl:text>GlassRockDb5.gif</xsl:text>
          ...
          <!--<xsl:value-of select="person/twitterid"/>-->
          </xsl:element>
       </span></td>
        ...
        <td width="10%"><span class="h44">
           <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:value-of select="concat('corp_by_id/', person/id, '.xml')"/>
            </xsl:attribute>
           <xsl:element name="img">
             <xsl:attribute name="src">
                <xsl:text>GlassRockDb5.gif</xsl:text>
        ...
        <td width="10%"><span class="h44">   
           <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:value-of select="concat('indus_by_id/', person/id, '.xml')"/>
            </xsl:attribute>
           <xsl:element name="img">
             <xsl:attribute name="src">
                <xsl:text>GlassRockDb5.gif</xsl:text>
...
<div class="c5"></div>
<!--BREAK-->
<div class="row show-grid span9 c5" title=""><span class="h3">Unless otherwise mentioned, all records courtesy of the fellows at <a href="https://govtrack.us">GovTrack</a> Many thanks</span></div>
<div class="row show-grid span9 c7" title=""><span class="h3">Special thanks to the <a href="https://www.anonops.com">AnonOps</a> Network</span></div>
</div>
<!-- /container -->
<!-- Le javascript
    ================================================== -->
